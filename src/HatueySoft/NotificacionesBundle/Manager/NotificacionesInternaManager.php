...
use HatueySoft\NotificacionesBundle\Entity\Notificacion;
...
class NotificacionesInternaManager {
    ...
    public function sendNotificaciones($codigoNotif, $params)
    {
        ...
        $config = $this->em->getRepository('NotificacionesBundle:ConfigNotificaciones')->findOneByCodigo($codigoNotif);
        if(!$config)
            return false;
    ...
    public function createNotificacionInstance($asunto, $codigo , $usuario, $params, $tipo='info')
    {
        try{
            ...
            $notif->setMensaje($this->templating->render(
                sprintf('NotificacionesBundle:InternaTemplates:%s_template.html.twig', $codigo),
                $params));
            $notif->setTipo($tipo);
    ...
    public function cambiarEliminado($notificacion)
    {
        try{
            ...
            {
                $entity = $this->em->find('NotificacionesBundle:Notificacion', $notificacion);
    ...
    public function getNotificacionesUsuario(Usuario $usuario){
        if ($usuario->getUsername() != 'admin') {
            $dql = 'SELECT n FROM HatueySoftNotificacionesBundle:Notificacion n INNER JOIN n.usuario u WHERE n.deleted IS NULL AND u.id = :usuarioid ORDER BY n.created DESC';
    ...
    public function countNotificacionesUsuario(UserInterface $usuario, $sinLeer)
    {
        if ($usuario->getUsername() != 'admin'){
            if ($sinLeer) {
                $dql = 'SELECT COUNT(n.id) FROM HatueySoftNotificacionesBundle:Notificacion n INNER JOIN n.usuario u WHERE n.deleted IS NULL AND n.leido = 0 AND u.id = :usuarioid';
            }
            else{
                $dql = 'SELECT COUNT(n.id) FROM HatueySoftNotificacionesBundle:Notificacion n INNER JOIN n.usuario u WHERE n.deleted IS NULL AND u.id = :usuarioid';
            }
