...
use HatueySoft\NotificacionesBundle\Entity\NotificacionCorreo;
...
class NotificacionesCorreoManager {
    ...
    public function sendCorreos($codigoNotif, $params)
    {
        ...
        if($config->getActivo() && $config->getNotificacionCorreo())
        {
            ...
            //Salvar el correo
            $notif = new NotificacionCorreo();
            $notif->setAsunto($args['subject']);
            $notif->setCodigo($codigoNotif);
            $notif->setCorreosEnviados($config->getCorreosDefinidos());
            $notif->setMensaje($params['cuerpo']);
            ...
            $message = \Swift_Message::newInstance()
                ...
                ->setBody(
                    $this->templating->render(
                        sprintf('NotificacionesBundle:CorreoTemplates:%s_template.html.twig', $args['codigo']),
                        $params
                    )
                );
