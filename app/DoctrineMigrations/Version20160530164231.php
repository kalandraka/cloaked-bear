...
class Version20160530164231 extends AbstractMigration
{
    ...
    public function up(Schema $schema)
    {
        ...
        $this->addSql('UPDATE d_bitacora_almacen b SET b.qty = b.movement_qty WHERE b.movement_type IN (\'V+\', \'M+\', \'P+\', \'I+\', \'D+\')');
        $this->addSql('UPDATE d_bitacora_almacen b SET b.qty = - b.movement_qty WHERE b.movement_type IN (\'V-\', \'M-\', \'P-\', \'I-\', \'D-\')');
        $this->addSql('ALTER TABLE d_bitacora_serial ADD qty INT DEFAULT NULL');
        $this->addSql('UPDATE d_bitacora_serial bs SET bs.qty = 1 WHERE bs.movement_type IN (\'V+\', \'M+\', \'P+\', \'I+\', \'D+\')');
        $this->addSql('UPDATE d_bitacora_serial bs SET bs.qty = -1 WHERE bs.movement_type IN (\'V-\', \'M-\', \'P-\', \'I-\', \'D-\')');
...
DROP TEMPORARY TABLE IF EXISTS  bitacoras_serial_producto_almacen;
CREATE TEMPORARY TABLE IF NOT EXISTS bitacoras_serial_producto_almacen
SELECT qty , serial  from d_bitacora_serial b
WHERE b.product_id = product_id and b.warehouse_id= warehouse_id ;

select res.serial from
(
 select serial, sum(qty) as cant
from bitacoras_serial_producto_almacen
group by serial
having cant>0
 ) res ;
...
END');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('ALTER TABLE d_bitacora_serial DROP qty');
